<script>
  ...
  export let selectedExport = false;
  export let exportDocumentPrefix = "Export";
  ...
  function selectForExport(id) {
    if (selectedForExport.indexOf(id) !== -1) {
      selectedForExport.splice(selectedForExport.indexOf(id), 1);
    } else {
      selectedForExport.push(id);
    }
  }

  function getSelectedRows(rows) {
    if (!selectedExport) return rows;
    let _rows = [];
    for (let rowIndex of selectedForExport) {
  ...
  <div class="table-header">
    ...
    <div class="actions">
      {#each customButtons as button, x}
        {#if !button.hide}
          <a href="#!" class="waves-effect btn-flat nopadding" on:click={click}>
      ...
      {#if printable}
        <a
          ...
          on:click={e => {
            print(getSelectedRows(rows), columns);
            e.preventDefault();
          }}>
      ...
      {#if exportable}
        <a
          ...
          on:click={e => {
            exportExcel(getSelectedRows(rows), columns, exportDocumentPrefix);
            e.preventDefault();
          }}>
